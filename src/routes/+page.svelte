...
<div class="card bg-base-200 text-center">
	<div class="card-body">
		<h1 class="text-center text-2xl font-bold">{m.general_welcome()}</h1>
		...
		{#if !data.session}
			...
			<span class="btn btn-lg btn-fsek m-1" slot="submitButton"
				...
				><span class="btn btn-lg btn-fsek-disabled btn-fsek m-1"
					><span class="size-6"
						><svg height="3em" width="3em" fill="#000" focusable="false" aria-hidden="true">
							<path
								d="M570.741,259.198H449.716
		...
		{:else}
			<SignOut>
				<span class="btn btn-secondary" slot="submitButton">{m.nav_signout()}</span>
			</SignOut>
		{/if}
	</div>
</div>
